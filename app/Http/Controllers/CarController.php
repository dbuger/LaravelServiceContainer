...
class CarController extends Controller
{
    ...
    public function __construct(Request $request, ICarRepository $repo, IBrandRepository $bRepo)
    {
        $this->repo = $repo;
        $this->bRepo = $bRepo;
    ...
    {
        return response()->json($this->repo->find($id,['*'],['brand']));
    }

    public function getBrand($id): JsonResponse
    {
        $car = response()->json($this->repo->find($id));
        $brand = $this->bRepo->find($car->brand_id);
